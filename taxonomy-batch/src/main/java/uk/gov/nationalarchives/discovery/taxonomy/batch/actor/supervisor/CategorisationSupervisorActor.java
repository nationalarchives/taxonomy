...
import uk.gov.nationalarchives.discovery.taxonomy.batch.actor.CategorisationStatusEnum;
...
import uk.gov.nationalarchives.discovery.taxonomy.common.domain.service.actor.Shutdown;
        ...
        if (currentEpic instanceof CategoriseAllDocumentsEpic) {
            if (hasDocumentsLeftFromDocIndex()) {
                ...
                if (nextDocReferences == null || nextDocReferences.length == 0) {
                    ...
                    logger.info("CATEGORISATION TERMINATED ON SUPERVISOR SIDE. Shut down and ask workers to shut down after " +
                                    "completing their tasks",
                            status);


                    for (ActorRef actorRef : workers) {
                        actorRef.tell(new Shutdown(), getSelf());
                    }
